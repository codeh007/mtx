//  cloudfalred worker 实现 基于 vless 一些的网络代理 可用 v2rayN 客户端连接
//  代码来自： https://blog.rnaan.com/post/22
//  github: https://github.com/zizifn/edgetunnel/blob/main/src/worker-vless.js
//          socks5版： https://github.com/zizifn/edgetunnel/blob/main/src/worker-with-socks5-experimental.js

import { connect } from "cloudflare:sockets";

// 打开 节点页面 https://mydomain.com/15a89a64-1605-4fa9-a3d8-54d616ed5544

// How to generate your own UUID:
// [Windows] Press "Win + R", input cmd and run:  Powershell -NoExit -Command "[guid]::NewGuid()"
// 在v2rayN中，点击 添加 vless 服务器，弹出框中，生成uid选项，生成的id跟这里的userID值对应。
let userID = "15a89a64-1605-4fa9-a3d8-54d616ed5544";
...
let dohURL =
  "https://sky.rethinkdns.com/1:-Pf_____9_8A_AMAIgE8kMABVDDmKOHTAKg="; // https://cloudflare-dns.com/dns-query or https://dns.google/dns-query
      ...
      if (!upgradeHeader || upgradeHeader !== "websocket") {
        ...
        switch (url.pathname) {
          ...
          case `/sub/${userID_Path}`: {
            ...
            const vlessSubConfig = createVLESSSub(
              userID,
              request.headers.get("Host"),
            );
...
export async function uuid_validator(request: any) {
...
export async function hashHex_f(string: any) {
 ...
 */
async function vlessOverWSHandler(request: any) {
  ...
  const log = (
    ...
    /** @type {string | undefined} */ event?: string,
  ) => {
    console.log(
...
async function handleTCPOutBound(
   ...
   */
  async function connectAndWrite(address: any, port: any) {
 ...
 */
function processVlessHeader(vlessBuffer: any, userID: any) {
  ...
  isValidUser =
    uuids.some((userUuid: any) => slicedBufferString === userUuid.trim()) ||
 ...
 */
function base64ToArrayBuffer(base64Str: any) {
  if (!base64Str) {
...
function unsafeStringify(arr: any, offset = 0) {
  return (
 ...
 * @returns {string}
 */
function getVLESSConfig(userIDs: any, hostName: any) {
...
<a href='v2rayng://install-config?url=${encodeURIComponent(
  ...
  <head>
    ...
    `vless://${userIDs.split(",")[0]}@${hostName}${commonUrlPart}`,
  )}' />
...
function createVLESSSub(userID_Path: any, hostName: any) {
  ...
  const output = userIDArray.flatMap((userID: any) => {
