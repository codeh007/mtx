...
import { useCompletionInput } from "../hooks";
import { MemoizedReactMarkdown } from "../markdown";
import { ArticleFaceItem } from "../ui-core/ui-core";

export interface AiCompletionProps {
	// biome-ignore lint/suspicious/noExplicitAny: <explanation>
	userInput: any;
	content?: string;
	variant?: "sidebar" | "mainscreen";
}
export const AiCompletion = (props: AiCompletionProps) => {
	const { userInput, variant = "sidebar" } = props;
	const { submitText, completion, artifacts } = useCompletionInput();
	...
	const content = props.content || completion;
	const className = "";
	return (
		<>
			{variant === "sidebar" && (
				<div className=" rounded-lg p-1 w-full">
					...
					{/* {completion} */}
					<ChatbotArtifactsView artifacts={artifacts} />
				</div>
			)}
			{props.variant === "mainscreen" && (
							...
							<div className="flex-col gap-1 md:gap-3">
								<div className="flex max-w-full flex-col flex-grow">
									<div
										...
										data-message-id="8ff633c5-64b7-41fd-bec6-5332169243ae"
										dir="auto"
...
interface ChatbotArtifactsViewProps {
	// biome-ignore lint/suspicious/noExplicitAny: <explanation>
	artifacts?: any[];
}
export const ChatbotArtifactsView = (props: ChatbotArtifactsViewProps) => {
	const { artifacts } = props;
	return (
		<div>
			{artifacts?.map((item, i) => {
				// biome-ignore lint/suspicious/noArrayIndexKey: <explanation>
				return <ArticleFaceItem key={i} {...item} />;
			})}
		</div>
	);
};

// const usedLocalChatId = () => {
// 	return useQuery({
