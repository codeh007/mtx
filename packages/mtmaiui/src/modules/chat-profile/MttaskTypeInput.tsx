...
export const MttaskTypeInput = (props: InputProps) => {
  return (
    <Suspense>
      <MtErrorBoundary>
        <MttaskTypeInputImpl {...props} />
      </MtErrorBoundary>
    </Suspense>
  );
};

const MttaskTypeInputImpl = (props: InputProps) => {
  const [open, setOpen] = React.useState(false);
  const [value, setValue] = React.useState("");

  const placeholder = "选择任务类型...";
  const emptyLabel = props.placeholder || "选择任务类型";
  const query = useSuspenseQuery({
    ...tasksTaskTypesOptions(),
  });
