  ...
  private async init(retries = 0) {
    ...
    if (retries > 0) {
      this.client.logger.info(
        `Retrying in ... ${DEFAULT_EVENT_LISTENER_RETRY_INTERVAL} seconds`,
      );
    ...
    try {
      ...
      this.listener = this.client.client.subscribeToWorkflowRuns(
        this.request(),
        {
