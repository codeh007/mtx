...
const stepEventTypeMap: Record<ResourceEventType, RunEventType | undefined> = {
  [ResourceEventType.RESOURCE_EVENT_TYPE_STARTED]:
    RunEventType.STEP_RUN_EVENT_TYPE_STARTED,
  ...
  [ResourceEventType.RESOURCE_EVENT_TYPE_FAILED]:
    RunEventType.STEP_RUN_EVENT_TYPE_FAILED,
  ...
  [ResourceEventType.RESOURCE_EVENT_TYPE_STREAM]:
    RunEventType.STEP_RUN_EVENT_TYPE_STREAM,
  [ResourceEventType.RESOURCE_EVENT_TYPE_UNKNOWN]: undefined,
  [ResourceEventType.UNRECOGNIZED]: undefined,
};

const workflowEventTypeMap: Record<
  ResourceEventType,
  ...
  async listenLoop(listenerFactory: () => AsyncIterable<WorkflowEvent>) {
    ...
    try {
      for await (const workflowEvent of listener) {
        const eventType =
          resourceTypeMap[workflowEvent.resourceType]?.[
...
export class ListenerClient {
  ...
  async streamByAdditionalMeta(
    ...
    const listener = RunEventListener.forAdditionalMeta(
      key,
      value,
      this.client,
    );
