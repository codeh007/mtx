...
export class Worker {
  ...
  constructor(
    ...
    this.killing = false;
    this.handle_kill =
      options.handleKill === undefined ? true : options.handleKill;

    this.logger = new Logger(
      `Worker/${this.name}`,
      this.client.config.log_level,
    );
  }
      ...
      const concurrency: WorkflowConcurrencyOpts | undefined =
        workflow.concurrency?.name || workflow.concurrency?.expression
          ? {
              ...
              limitStrategy:
                workflow.concurrency.limitStrategy ||
    ...
    try {
      ...
      const success = async (result: any) => {
        ...
        try {
          ...
          this.logger.error(
            `Could not send completed action event: ${actionEventError.message}`,
          );
          ...
          try {
            ...
            this.logger.error(
              `Could not send failed action event: ${failureEventError.message}`,
            );
          }

          this.logger.error(
            `Could not send action event: ${actionEventError.message}`,
          );
        }
      };
      ...
      const failure = async (error: any) => {
        this.logger.error(
          `Step run ${action.stepRunId} failed: ${error.message}`,
        );
  ...
  async handleStartGroupKeyRun(action: Action) {
    ...
    try {
      ...
      if (!key) {
        this.logger.error(
          `No group key run id provided for action ${actionId}`,
        );
        return;
      }
  ...
  async exitGracefully(handleKill: boolean) {
    ...
    await Promise.all(
      Object.values(this.futures).map(({ promise }) => promise),
    );
  ...
  async handleAction(action: Action) {
    ...
    if (type === ActionType.START_STEP_RUN) {
      ...
      this.logger.error(
        `Worker ${this.name} received unknown action type ${type}`,
      );
    }
  }
