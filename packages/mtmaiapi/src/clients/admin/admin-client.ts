...
import type { ListenerClient } from "../listener/listener-client";
...
export class AdminClient {
  ...
  async putWorkflow(workflow: CreateWorkflowVersionOpts) {
    try {
      return await retrier(
        async () => this.client.putWorkflow({ opts: workflow }),
        this.logger,
      );
    } catch (e: any) {
  ...
  runWorkflow<Q = object, P = object>(
    ...
    try {
      if (
        ...
        !workflowName.startsWith(this.config.namespace)
      ) {
  ...
  runWorkflows<Q = object, P = object>(
    ...
    const workflowRequests = workflowRuns.map(
      ({ workflowName, input, options }) => {
        let computedName = workflowName;
  ...
  async schedule_workflow(
    name: string,
    options?: { schedules?: Date[]; input?: object },
  ) {
