...
export function ChatHistoryClient({ initialChats, initialNextOffset }: ChatHistoryClientProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto mb-2 relative">
      {!chats?.length && !isLoading && nextOffset === null ? (
        <div className="px-2 text-foreground/30 text-sm text-center py-4">No search history</div>
      ) : (
        <SidebarMenu>
