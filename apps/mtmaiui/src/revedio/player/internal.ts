...
class RevideoPlayer extends HTMLElement {
  public static get observedAttributes() {
    return ["playing", "variables", "looping", "fps", "quality", "width", "height", "volume"];
  }

  private get fps() {
    const attr = this.getAttribute("fps");
    return attr ? Number.parseFloat(attr) : (this.defaultSettings?.fps ?? 60);
  }

  private get quality() {
    const attr = this.getAttribute("quality");
    return attr ? Number.parseFloat(attr) : (this.defaultSettings?.resolutionScale ?? 1);
  }

  private get width() {
    const attr = this.getAttribute("width");
    return attr ? Number.parseFloat(attr) : (this.defaultSettings?.size.width ?? 0);
  }

  private get height() {
    const attr = this.getAttribute("height");
    return attr ? Number.parseFloat(attr) : (this.defaultSettings?.size.height ?? 0);
  }
  ...
  private defaultSettings: ReturnType<typeof getFullPreviewSettings> | undefined;
  private abortController: AbortController | null = null;
  private playing = false;
  private stage = new Stage();

  private time = 0;
  private duration = 0; // in frames
  ...
  private render = async () => {
    if (this.player && this.project) {
      ...
      console.log("no player or project");
    }
  };
