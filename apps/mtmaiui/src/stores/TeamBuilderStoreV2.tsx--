...
export const TeamBuilderV2Provider = (props: AppProviderProps) => {
  ...
  const componentsQuery = useQuery({
    ...teamGetOptions({
      path: {
        tenant: tid,
        team: props.componentId,
      },
    }),
    enabled: !!tid && !!props.componentId,
  });

  const store = useMemo(() => {
    const store = createTeamBuilderStore({
      ...etc,
      team: componentsQuery.data,
      upsertComponent,
      tid,
      nav,
    });
...
export function useTeamBuilderStoreV2(): TeamBuilderState;
export function useTeamBuilderStoreV2<T>(
...
export function useTeamBuilderStoreV2<T>(
  ...
  if (!store)
    throw new Error("useTeamBuilderStoreV2 must in TeamBuilderV2Provider");
  if (selector) {
