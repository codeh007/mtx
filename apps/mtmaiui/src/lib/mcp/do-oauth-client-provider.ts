...
export class DurableObjectOAuthClientProvider implements AgentsOAuthProvider {
  ...
  async clientInformation(): Promise<OAuthClientInformation | undefined> {
    ...
    return (
      (await this.storage.get<OAuthClientInformation>(this.clientInfoKey(this.clientId))) ??
      undefined
    );
  }
  ...
  async codeVerifier(): Promise<string> {
    const codeVerifier = await this.storage.get<string>(this.codeVerifierKey(this.clientId));
    if (!codeVerifier) {
