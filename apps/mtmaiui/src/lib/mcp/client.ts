...
export class MCPClientManager {
  ...
  async connect(
    ...
    if (authUrl && options.transport?.authProvider?.redirectUrl) {
      this.callbackUrls.push(options.transport.authProvider.redirectUrl.toString());
    }
  ...
  async handleCallbackRequest(req: Request) {
    ...
    if (this.mcpConnections[serverId].connectionState !== "authenticating") {
      throw new Error("Failed to authenticate: the client isn't in the `authenticating` state");
    }
   ...
   */
  getPrompt(params: GetPromptRequest["params"] & { serverId: string }, options: RequestOptions) {
