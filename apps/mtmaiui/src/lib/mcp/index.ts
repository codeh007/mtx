...
class McpStreamableHttpTransport implements Transport {
  ...
  async send(message: JSONRPCMessage) {
    ...
    if (isJSONRPCResponse(message) || isJSONRPCError(message)) {
      ...
      if (!websocket) {
        throw new Error(`Could not find WebSocket for message id: ${message.id}`);
      }
...
> extends DurableObject<Env> {
  ...
  async onStart() {
    ...
    this.#transportType = (await this.ctx.storage.get("transportType")) as TransportType;
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    switch (path) {
      ...
      default:
        return new Response("Internal Server Error: Expected /sse or /streamable-http path", {
          status: 500,
        });
    }
  }
  ...
  async onMessage(connection: Connection, event: WSMessage) {
    ...
    if (this.#transportType !== "streamable-http") {
      const err = new Error("Internal Server Error: Expected streamable-http protocol");
  ...
  static serve(
    path: string,
    { binding = "MCP_OBJECT", corsOptions }: { binding?: string; corsOptions?: CORSOptions } = {},
  ) {
    let pathname = path;
    if (path === "/") {
      pathname = "/*";
