...
export function createSearchTool(fullModel: string) {
  return tool({
    ...
    execute: async ({
      ...
      const minResults = 10;
      const effectiveMaxResults = Math.max(max_results || minResults, minResults);
      ...
      const effectiveSearchDepthForAPI =
        searchAPI === "searxng" && process.env.SEARXNG_DEFAULT_DEPTH === "advanced"
          ? "advanced"
          : effectiveSearchDepth || "basic";

      console.log(`Using search API: ${searchAPI}, Search Depth: ${effectiveSearchDepthForAPI}`);
